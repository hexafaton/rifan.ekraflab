...
async function validateVoucher(items, code) {
  try {
    const response = await fetch("/api/validate-voucher", {
      ...
      body: JSON.stringify({ items, voucherCode: code }),
    });

    if (!response.ok) {
      throw new Error("Gagal memvalidasi voucher");
    }

    return await response.json();
  } catch (error) {
    console.error("validateVoucher error:", error);
    const totalFallback = items.reduce((sum, item) => {
...
async function renderPaymentPage() {
  ...
  const voucherResult = voucherCode
    ? await validateVoucher(cartItems, voucherCode)
    : { valid: false, discountAmount: 0, discountedTotal: totalAmount };
...
async function handlePaymentSubmit(event) {
  ...
  const voucherResult = voucherCode
    ? await validateVoucher(cartItems, voucherCode)
    : { valid: false, discountAmount: 0, discountedTotal: originalTotal };
